...
sequenceDiagram
    ...
    U-->>P: UserDetails
    P->>E: matches(rawPwd, encodedPwd)
    E-->>P: true/false
    alt 인증 성공
        P-->>M: Authentication (authenticated=true)
        M-->>F: Authentication
        F->>S: setAuthentication(auth)
        F-->>C: 200 OK + Session/Token
    else 인증 실패
        P-->>F: AuthenticationException
        F-->>C: 401
    end
```

## CustomAuthenticationFilter 전체 코드 예시 (Spring Security 6)
```java
public class JsonUsernamePasswordAuthFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public JsonUsernamePasswordAuthFilter(AuthenticationManager manager,
                                          SecurityContextRepository repository,
                                          ObjectMapper objectMapper) {
        super(new AntPathRequestMatcher("/api/login", "POST"), manager);
    ...
    protected void successfulAuthentication(HttpServletRequest req, HttpServletResponse res,
        ...
        // Spring Security 6: 자동 저장이 안 되므로 명시적으로 저장
...
## CustomAuthenticationProvider 코드 예시
```java
@Component
...
## SecurityConfig 등록 예시 (Spring Security 6)
```java
@Configuration
@EnableWebSecurity
@RequiredArgsConstructor
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http,
        ...
        var loginFilter = new JsonUsernamePasswordAuthFilter(manager, contextRepository, objectMapper);
...
| Grant Type | 사용 주체 | 흐름 요약 | 적합 |
|---|---|---|---|
| Authorization Code | 서버 사이드 웹 앱 | 인가 코드 → AccessToken 교환 | 가장 안전, 일반 웹 |
| Authorization Code + PKCE | SPA, 모바일 | 코드 검증자(verifier)로 가로채기 방지 | SPA, 네이티브 앱 |
| Client Credentials | 서버 간 통신 | 사용자 없음, 클라이언트 자격증명만 | 백엔드-백엔드 |
| Refresh Token | 토큰 갱신 | RefreshToken으로 AccessToken 재발급 | 모든 곳에서 보조 |
| Resource Owner Password (deprecated) | - | 사용자 ID/PW를 클라이언트가 직접 받음 | 사용 비추천 |
| Implicit (deprecated) | - | URL fragment로 토큰 직접 발급 | PKCE로 대체 |

## 보안 모범 사례 체크리스트
- **비밀번호**: 항상 BCrypt(또는 Argon2) 같은 단방향 해시. 평문/MD5/SHA-1 금지.
- **세션**: HttpOnly, Secure, SameSite=Lax 또는 Strict 쿠키 설정.
- **CSRF**: 상태 변경 요청(POST/PUT/DELETE)에 토큰 검증. SPA + JWT라면 CSRF가 의미 없으므로 disable 가능하지만, 그땐 XSS 방어가 필수.
- **XSS**: 출력 시 escape, Content-Security-Policy 헤더 설정.
- **JWT 사용 시**: 짧은 AccessToken + RefreshToken 회전, 시그니처 검증, `none` 알고리즘 거부, 민감 정보 클레임에 담지 않기.
- **Brute Force 방어**: 로그인 실패 시 지연·계정 잠금, IP 기반 rate limit.
- **HTTPS 강제**: HSTS 헤더 + HTTP는 redirect.
